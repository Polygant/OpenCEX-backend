...
def generate_tenz_multisig_keeper(log=None) -> Tuple[OrderedDict, Keeper]:
    from cryptocoins.coins.tenz.service import TENZCoinService
    service = TENZCoinService()
    tenz = Bitcoin()  # Since Tenzura is Bitcoin-compatible, we can use Bitcoin class
    ...
    is_segwit = not getattr(settings, 'TENZ_ADDRESS_LEGACY', False)

    pub_keys = [ad1.public_key, ad2.public_key, ad3.public_key]
    if is_segwit:
        script, address = tenz.mk_multsig_segwit_address(*pub_keys, num_required=2)
    else:
        script, address = tenz.mk_multsig_address(*pub_keys, num_required=2)

    '''
    create keeper in admin panel and add script to "keeper.extra" {"redeem_script":"script"}
    use ad3 private key in keeper
    import address to tenz node 
    ...
    user_wallet = UserWallet.objects.create(
        ...
        blockchain_currency=TENZ_CURRENCY,
        address=address,
