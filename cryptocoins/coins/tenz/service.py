...
from cryptocoins.coins.tenz import TENZ_CURRENCY
...
from cryptocoins.utils.tenz import tenz2sat
from lib.cipher import AESCoderDecoder
from lib.helpers import to_decimal


class TENZCoinService(BitCoreCoinServiceBase):
    CURRENCY = TENZ_CURRENCY
    GAS_CURRENCY = settings.ETH_TX_GAS
    node_config = settings.NODES_CONFIG['tenz']
    cold_wallet_address = settings.TENZ_SAFE_ADDR
    const_fee = 0.00003
    CRYPTO_COIN = Bitcoin()
    ...
    def get_transfer_fee(self, size):
        ...
        self.log.info(f'Fee = {s_p_b} Sat/b * {size} bytes / 10**8 = {fee} TENZ')
        return to_decimal(max(fee, to_decimal(self.const_fee)))

    def get_sat_per_byte(self):
        return sat_per_byte_cache.get('tenzura', settings.SAT_PER_BYTES_MIN_LIMIT)
    ...
    def prepare_outs(outs: dict) -> list:
        return [
            {
                'address': address,
                'value': int(tenz2sat(to_decimal(amount)))
            }
        ...
        return [
            {
                ...
                'value': int(tenz2sat(to_decimal(item['amount'])))
            }
            for item in inputs
        ]
    ...
    def check_tx_for_deposit(self, tx_data):
        ...
        for addr, amount in outputs_amount.items():
            ...
            if ScoreManager.need_to_check_score(tx_id, addr, amount, self.currency.code):
                ...
                process_deffered_deposit.apply_async((tx_id, addr, amount, self.currency.code), queue='tenz', countdown=defer_time)
            else:
